...
parser.add_argument('-v','--verbose', help='Description for bar argument', required=False, default='False')
...
class Agent:
    ...
    def createModel(self):
        if os.path.isfile('agent_lstm_basic_model.h5py'):
            model = keras.models.load_model('agent_lstm_basic_model.h5py')
        else:
            model = keras.models.Sequential()
            # model.add(keras.layers.Flatten(input_shape=(40, 70)))

            model.add(keras.layers.CuDNNLSTM(256, input_shape=(1, 2800), return_sequences=True))
            model.add(keras.layers.Dropout(0.2))

            model.add(keras.layers.CuDNNLSTM(128, input_shape=(1, 2800), return_sequences=True))
            model.add(keras.layers.Dropout(0.2))
    ...
    def preprocess(self, screen):
        ...
        screen = screen.reshape(1, 1, 2800)
        return screen
            ...
            for episode in range(1, self.episodes):
                ...
                while True:
                    ...
                    if isAlive:
                        ...
                        reward = penalty
                    screen = nextScreen
                    self.episodeMoves.append([screen, nextScreen, reward, isAlive, action])
                    if self.verbose:
                ...
                self.train()
                if episode % 40 == 0:
                    self.model.save('agent_lstm_basic_model.h5py')
